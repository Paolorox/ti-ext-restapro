<div class="panel-body px-0 pb-0">
    @if (!isset($formModel) || !$formModel->exists)
        <div class="alert alert-info">
            <i class="fa fa-info-circle me-2"></i> Save the unit first to see linked ingredients.
        </div>
    @else
        @php
            $ingredients = $formModel->ingredients()->with('category')->get();
        @endphp

        <div class="d-flex justify-content-between align-items-center px-4 mb-3">
            <h5 class="mb-0 fw-bold text-dark" style="font-family: 'Inter', sans-serif;">Ingredients using {{ $formModel->name }}</h5>
        ...
        @if($ingredients->isEmpty())
            <div class="text-center py-5">
                ...
                <p class="text-muted fw-medium">No ingredients use this unit.</p>
            </div>
        @else
                ...
                <table class="table table-hover align-middle mb-0" style="font-family: 'Inter', sans-serif;">
                    <thead style="background: rgba(248, 250, 252, 0.8);">
                        <tr>
                            ...
                            <th class="text-muted fw-semibold text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px; padding: 1rem 1.5rem;">Category</th>
                    ...
                    <tbody>
                        @foreach($ingredients as $ingredient)
                            <tr style="transition: all 0.2s ease;">
                                ...
                                <td style="padding: 1rem 1.5rem;">
                                    <span class="text-muted" style="font-size: 0.85rem;">{{ $ingredient->category ? $ingredient->category->name : '-' }}</span>
                                </td>
                                <td class="text-end" style="padding: 1rem 1.5rem;">
                                    ...
                                    <span class="text-muted ms-1" style="font-size: 0.85rem;">{{ $formModel->abbreviation }}</span>
                                </td>
                                <td class="text-end" style="padding: 1rem 1.5rem;">
